...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to input image (optional, for single image detection)",
    )
    ...
    while True:
        ...
        if not ret:
            ...
            # Run detection
        x, pad = detector.preprocess(frame)
        if detector.int8:
        ...
        if detector.int8:
            y = (y.astype(np.float32) - detector.out_zero_point) * detector.out_scale

        output_frame = detector.postprocess(frame, y, pad)

            # Display result
        cv2.imshow("YOLOv8 TFLite Detection", output_frame)
    ...
    #     cv2.imshow("YOLOv8 TFLite Detection", result)
